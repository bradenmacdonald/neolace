...
import { MergeSchemaParameters, SchemaParameters, SchemaResolveType, SchemaReturnType, SchemaValidatorFunction, } from "./schema/io.ts";
...
function schema<S>(schema: S, opts?: ErrorLike<SchemaParameters<S>> | {
    error?: ErrorLike<SchemaParameters<S>>;
    strict?: boolean;
}): ValidatorProxy<Validator<SchemaValidatorFunction<S>>> {
...
schema.either = function <A, S>(...candidates: [A, ...S[]]): ValidatorProxy<Validator<FunctionType<SchemaReturnType<A> | SchemaReturnType<S>, MergeSchemaParameters<SchemaParameters<A | S>>>>> {
    return new Validator(eitherSchema(...(candidates as [unknown]))).proxy() as unknown as ValidatorProxy<Validator<FunctionType<SchemaReturnType<A> | SchemaReturnType<S>, MergeSchemaParameters<SchemaParameters<A | S>>>>>;
};
schema.merge = function (...args: [unknown, ...unknown[]]): ValidatorProxy<Validator<FunctionType>> {
    return new Validator(mergeSchemas(...(args as [unknown]))).proxy() as ValidatorProxy<Validator<FunctionType>>;
};
schema.enum = function <E extends Enum<E>, P extends FunctionParameters = [E[keyof E]]>(value: E, error?: ErrorLike<P>): ValidatorProxy<Validator<FunctionType<E[keyof E], P>>> {
