...
import { group, test, setTestIsolation, assertRejects, assertEquals, beforeAll } from "neolace/lib/tests.ts";
    ...
    group("files()", () => {
        ...
        test(`It gives data about the files associated with an entry`, async () => {
            ...
            await graph.runAsSystem(ApplyEdits({siteId, edits: [
                ...
                // Create an entry:
                {code: "CreateEntry", data: {id: entryId, type: entryType, name: "Test With Files", friendlyId: "files-test", description: "An Entry with Files, for Testing"}},
            ]}));
        ...
        test(`It gives an empty list value when used with non-file entries`, async () => {
            const expression = new Files(new LiteralExpression(new EntryValue(
                defaultData.entries.ponderosaPine.id
            )), {});

            assertEquals(await evalExpression(expression), new PageValue([], {startedAt: 0n, pageSize: 10n, totalCount: 0n}));
        });
