    ...
    async editFeature(entryId, editData, tx) {
        const query = C`
            ...
            ON CREATE SET
                propData.id = ${VNID()},
                propData.articleMD = ""
            WITH propData, propData.articleMD as oldArticleMD, ${editData.articleMD} as newArticleMD
            WITH propData, oldArticleMD, newArticleMD, newArticleMD<>oldArticleMD AS isDifferent
            SET propData += CASE WHEN isDifferent THEN {articleMD: newArticleMD} ELSE {} END
        `.RETURN({ "propData.id": Field.VNID, isDifferent: Field.Boolean, oldArticleMD: Field.String });
        ...
        return {
            ...
            oldValues: {
                articleMD: result.records[0].get("oldArticleMD"),
            },
        };
    },
    /**
