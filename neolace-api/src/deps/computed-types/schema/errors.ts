...
export type ErrorLike<P extends FunctionParameters = never> = string | ValidationError | ((...args: P) => string | ValidationError);
...
export function toError<P extends FunctionParameters>(error: ErrorLike<P>, ...args: P): ValidationError {
...
export function toPathErrors(errorLike: ErrorLike, path: ObjectPath): PathError[] {
    const error = toError(errorLike);
    if (Array.isArray(error.path)) {
        path = error.path;
    }
    if (error.errors?.length) {
        return ([] as PathError[]).concat(...error.errors.map((item) => toPathErrors(item.error, [...path, ...item.path])));
    }
    return [{ error, path }];
}
export function createValidationError<P extends FunctionParameters>(errors: PathError[], error: ErrorLike<P> | null | undefined, ...args: P): ValidationError {
    if (!error) {
        if (errors[0]) {
